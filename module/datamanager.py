...
class WebUIDataManager(DataManager):
    ...
    def __init__(self, rg):
        """Provide a Regenerator object that was used to restore objects from the broks

        :param rg: Regenerator
        """
    ...
    def get_hosts_synthesis(self, elts=None, user=None):
        ...
        if hosts:
            ...
            h['nb_problems'] = 0
            for host in hosts:
                if host.state.lower() in ['down', 'unreachable'] and \
                        not host.problem_has_been_acknowledged:
                    h['nb_problems'] += 1
                    logger.debug("[WebUI - datamanager] get_hosts_synthesis: %s: %s, %s, %s",
                                 host.get_name(), host.state, host.is_problem,
                                 host.problem_has_been_acknowledged)

            h['pct_problems'] = round(100.0 * h['nb_problems'] / h['nb_elts'], 1)
            h['nb_ack'] = sum(1 for host in hosts if host.is_problem and
                              host.problem_has_been_acknowledged)
            ...
            h['bi'] = 0
            for state in ['up', 'down', 'unreachable', 'pending', 'unknown',
    ...
    def get_services_synthesis(self, elts=None, user=None):
        ...
        if services:
            ...
            for service in services:
                if service.state.lower() in ['warning', 'critical'] and \
                        not service.problem_has_been_acknowledged:
                    s['nb_problems'] += 1
            ...
            s['nb_ack'] = sum(1 for service in services if service.is_problem and
                              service.problem_has_been_acknowledged)
    ...
    def search_hosts_and_services(self, search, user, get_impacts=True, sorter=None):
        """ Search hosts and services.
            ...
            All other methods should be based on this one.
        ...
        logger.debug("[WebUI - datamanager] search_hosts_and_services, search for %s in %d items",
                     search, len(items))
        ...
        for t, s in patterns:
            ...
            if t == 'name':
                ...
                for i in items:
                    if (pat.search(i.get_full_name())
                            or (i.__class__.my_type == 'host'
                                and i.alias and pat.search(i.alias))):
                        new_items.append(i)
                    else:
            ...
            if (t in ['h', 'host']) and s.lower() != 'all':
                ...
                for item in items:
                    logger.debug("[WebUI - datamanager] item %s is %s",
                                 item.get_name(), item.__class__)
            ...
            if (t in ['hg', 'hgroup']) and s.lower() != 'all':
                logger.debug("[WebUI - datamanager] searching for items in the hostgroup %s", s)
                group = self.get_hostgroup(s)
                if not group:
                    return []

                items = [i for i in items if group.get_name() in [h.get_name()
                                                                  for h in i.get_hostgroups()]]

            if (t in ['sg', 'sgroup']) and s.lower() != 'all':
                logger.debug("[WebUI - datamanager] searching for items in the servicegroup %s", s)
                group = self.get_servicegroup(s)
                if not group:
                    return []

                items = [i for i in items if i.__class__.my_type == 'service' and
                         group.get_name() in [s.get_name() for s in i.servicegroups]]

            # @mohierf: to be refactored!
            if (t in ['cg', 'cgroup']) and s.lower() != 'all':
                logger.info("[WebUI - datamanager] searching for items related "
                            "with the contactgroup %s", s)
                group = self.get_contactgroup(s, user)
                if not group:
                    return []

                contacts = [c for c in self.get_contacts(user=user) if c in group.members]
                items = list(set(itertools.chain(*[self._only_related_to(
                    items, self.rg.contacts.find_by_name(c)) for c in contacts])))
            ...
            if t == 'is':
                if s.lower() == 'ack':
                    ...
                    if s.startswith('s'):
                        s = s[1:]
                        if len(s) == 1:
                            items = [i for i in items if i.state_id == int(s) and
                                     i.state_type != 'HARD']
                        else:
                            items = [i for i in items if i.state == s.upper() and
                                     i.state_type != 'HARD']
                    elif s.startswith('h'):
                        s = s[1:]
                        if len(s) == 1:
        ...
        logger.debug("[WebUI - datamanager] search_hosts_and_services, found %d matching items",
                     len(items))
    ...
    def set_contactgroup_level(self, group, level, user):
        logger.debug("[WebUI - datamanager] set_contactgroup_level, group: %s, level: %d",
                     group, level)
    ...
    def get_contactgroups(self, user=None, name=None, parent=None, members=False):
        """ Get a list of known contacts groups
            ...
            :returns: List of elements related to the user
        """
        logger.debug("[WebUI - datamanager] get_contactgroups, name: %s, members: %s",
                     name, members)
        items = []
        if parent:
    ...
    def get_contactgroup(self, name, user=None, members=False):
        ...
        return self._only_related_to(self.get_contactgroups(user=user, name=name,
    ...
    def set_hostgroup_level(self, group, level, user):
        setattr(group, 'level', level)
        logger.debug("[WebUI - datamanager] set_hostgroup_level, group: %s, level: %d",
                     group, level)
    ...
    def get_hostgroup_hosts(self, name, user=None):
        ...
        if group:
            for host in group.get_hosts():
                logger.warning("[WebUI - datamanager] -> host: %s, contacts: %s",
                               host.get_name(), host.contacts)
                if user:
                    for contact in host.contacts:
                        if contact.contact_name == user.contact_name:
                            logger.info("[WebUI - relation], user is a contact "
                                        "through an hostgroup")
    ...
    def get_servicegroup(self, name, user=None, parent=None, members=False):
        ...
        return self._is_related_to(self.get_servicegroups(user=user, name=name, members=members),
                                   user)
    ...
    ##
    # Realms - not managed at all in the Regenerator, remove them here!
    ##
    ...
    def get_important_elements(self, user, type='all', sorter=worse_first):
        return self.search_hosts_and_services('bi:>2 ack:false type:%s' % type,
