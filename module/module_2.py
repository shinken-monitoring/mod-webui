...
class Webui_broker(BaseModule, Daemon):
    ...
    def main(self, stand_alone=False):
        ...
        try:
            ...
            webui_app.run(host=self.host, port=self.port, server='gevent', **self.serveropts)
        except Exception as e:
            if self.stand_alone:
    ...
    def put_brok(self, host_name, data_type, data, brok_type):
    ...
    # def manage_brok_loop(self):
        # logger.info("[*****WEBUI] manage_brok_thread START")

        # while not self.interrupted:
            # logger.info("[*****WEBUI] queue length 1: %s", self.to_q.qsize())
            # start = time.clock()
            # try:
            ...
            # else:
                # for b in l:
                    # b.prepare()
                    # self.manage_brok(b)
                # logger.info("[*****WEBUI] time to manage %s broks (%3.4fs)" % (len(l), time.clock() - start))
                # gevent.sleep(0.1)
