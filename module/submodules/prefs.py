...
class PrefsMetaModule(MetaModule):
    ...
    def __init__(self, modules, app):
        ...
        if modules:
            if len(modules) > 1:
                logger.warning('[WebUI] Too much prefs modules declared (%s > 1). '
                               'Using %s.' % (len(modules), modules[0]))
...
class MongoDBPreferences:
    ...
    def __init__(self, mod_conf):
        ...
        logger.info("[WebUI-MongoDBPreferences] Trying to open a Mongodb connection to %s, "
    ...
    def open(self):
        ...
        try:
            ...
            logger.info('[WebUI-MongoDBPreferences] searching connection test item '
                        'in the collection ...')
            u = self.db.ui_user_preferences.find_one({'_id': 'shinken-test'})
            if not u:
                # No document ... create a new one!
                logger.debug('[WebUI-MongoDBPreferences] not found connection test item '
                             'in the collection')
                r = self.db.ui_user_preferences.save({'_id': 'shinken-test',
                                                      'last_test': time.time()})
                logger.debug('[WebUI-MongoDBPreferences] result: %s', r)
                logger.info('[WebUI-MongoDBPreferences] updated connection test item')
            else:
                # Found document ... update!
                logger.debug('[WebUI-MongoDBPreferences] found connection test item '
                             'in the collection')
                r = self.db.ui_user_preferences.update({'_id': 'shinken-test'},
            ...
            logger.debug("[WebUI-MongoDBPreferences] Back trace of this kill: %s",
    ...
    def get_ui_common_preference(self, key):
        if not self.is_connected:
            if not self.open():
                logger.error("[WebUI-MongoDBPreferences] error during initialization, "
        ...
        if not e or key not in e:
            logger.debug("[WebUI-MongoDBPreferences] new parameter or not stored preference: %s",
                         key)
            return None
    ...
    def set_ui_user_preference(self, user, key, value):
        ...
        try:
            ...
            r = self.db.ui_user_preferences.update({'_id': user.contact_name},
                                                   {'$set': {key: value}})
            ...
            if not r:
                ...
                u = self.db.ui_user_preferences.find_one({'_id': user.contact_name})
                if not u:
                    logger.debug("[WebUI-MongoDBPreferences] No user entry for %s, "
                                 "I create a new one", user.contact_name)
                    self.db.ui_user_preferences.save({'_id': user.contact_name, key: value})
                else:  # ok, it was just the key that was missing, just update it and save it
                    u[key] = value
                    logger.debug("[WebUI-MongoDBPreferences] Just saving the new key "
                                 "in the user pref")
    ...
    def set_ui_common_preference(self, key, value):
        ...
        try:
            ...
            if not u:
                ...
                r = self.db.ui_user_preferences.update({'_id': 'shinken-global'},
                                                       {'$set': {key: value}})
        except Exception as e:
            logger.warning("[WebUI-MongoDBPreferences] Exception: %s", str(e))
            self.is_connected = False
