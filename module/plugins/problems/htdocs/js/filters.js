...
function refresh_new_search_div(){
    if (new_filters.length == 0) {
      $('#new_search').html('No filter selected ...');
      // Actions buttons are now useless...
      $('#remove_all_filters').animate({'opacity': 0.3}).addClass('disabled');
      $('#remove_all_filters2').animate({'opacity': 0.3}).addClass('disabled');
      $('#launch_the_search').animate({'opacity': 0.3});
      return;
    }

    // The actions buttons are now important :)
    $('#remove_all_filters').animate({'opacity': 1}).removeClass('disabled');
    $('#remove_all_filters2').animate({'opacity': 1}).removeClass('disabled');
    $('#launch_the_search').animate({'opacity': 1});

    s = 'New filters: <ul class="list-group list-inline">';
    $.each(new_filters, function(idx, f){
      ...
      s+= '<li class="list-group-item">'+t+c+'</li>';
    });
    s += '</ul>';
    $('#new_search').html(s);
...
function already_got_filter(type, name){
    if (current_filters.length == 0) {
      // Remove current filters button is now useless...
      $('#remove_all_filters2').animate({'opacity': 0.3}).addClass('disabled');
      return;
    }
    
    // Remove current filters button is now important :)
    $('#remove_all_filters2').animate({'opacity': 1}).removeClass('disabled');
