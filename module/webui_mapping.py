...
dm_mapping = {
    'Host': {
        ...
        # Ignored, because specific to Nagios WebUI
        'action_url': {
        ...
        'comments_with_info': {
            ...
            'function': lambda item, req: [sep_join(req, str(x.id), x.author, x.comment) for x in item.comments],  # REPAIRME MAYBE
            'datatype': list,
        },
        ...
        # List of links to ContactsGroup
        'contact_groups': {
        ...
        'custom_variables': {
            ...
            'function': lambda item, req: [sep_join(req, k[1:], v) for k, v in item.customs.iteritems()],
            'datatype': list,
            'api': True
        },
        'display_name': {
        ...
        'downtimes_with_info': {
            ...
            'function': lambda item, req: [sep_join(req, str(x.id), x.author, x.comment) for x in item.downtimes],  # REPAIRME MAYBE
            'datatype': list,
        },
        # Link to EventHandler
        'event_handler': {
            'description': 'Nagios command used as event handler',
            'function': lambda item, req: item.event_handler.get_name() if item.event_handler else None,
            # 'api': True
        },
        ...
        # Should be computed ... do not store.
        'impacts': {
        ...
        'in_check_period': {
            ...
            'function': lambda item, req: (item.check_period is None and [False] or [item.check_period.is_time_valid(time.time())])[0],
            'datatype': bool,
        },
        # Should be computed ... do not store.
        'in_notification_period': {
            'description': 'Whether this host is currently in its notification period (0/1)',
            'function': lambda item, req: (item.notification_period is None and [False] or [item.notification_period.is_time_valid(time.time())])[0],
            'datatype': bool,
        },
        'initial_state': {
        ...
        # Any interest in this ... I suppose so!
        'modified_attributes': {
        ...
        'next_notification': {
            ...
            # interval_length is an attribute of base class SchedulingItem ...
            # 'function': lambda item, req: int(item.last_notification + item.notification_interval * item.__class__.interval_length),  # CONTROLME
            # interval_length is an attribute of base class SchedulingItem ... consider as 1
            'function': lambda item, req: int(item.last_notification + item.notification_interval * 1),  # CONTROLME
            'datatype': int,
            'api': True
        },
        # Ignored, because specific to Nagios WebUI
        ...
        'notification_period': {
            ...
            'function': lambda item, req: item.notification_period.get_name() if item.notification_period else None,
            'api': True
        },
        ...
        'no_more_notifications': {
            ...
            'function': lambda item, req: False,  # REPAIRME, maybe ask both instance and class
            'datatype': bool,
        },
        'num_services': {
        ...
        'pnpgraph_present': {
            ...
            'function': lambda item, req: False,
            'datatype': int,
        },
        'poller_tag': {
        ...
        'services_with_info': {
            ...
            'function': lambda item, req: None,  # REPAIRME
            'datatype': list,
        ...
        'services_with_state': {
            ...
            'function': lambda item, req: [sep_join(req, x.get_name(), x.state_id, x.has_been_checked) for x in item.services],
            'datatype': list,
        },
        # Should be computed ...
        'source_problems': {
        ...
        'tags': {
            ...
            'function': lambda item, req: [sep_join(req, x) for x in item.tags],
            'datatype': list,
            'api': True
        },
        'total_services': {
        ...
        # Should be computed ... but interesting to have?
    ...
    'Service': {
        ...
        'host_state': {
            'description': 'The current state of the host (0: up, 1: down, 2: unreachable)',
            ##'function': lambda item, req: item.host.wui_state(req),
        },
        'host_state_type': {
            'description': 'Type of the current state (0: soft, 1: hard)',
        },
    ...
    'Hostgroup': {
        ...
        'members_with_state': {
            ...
            'function': lambda item, req: [sep_join(req, x.get_name(), x.state_id, x.has_been_checked) for x in item.members],
            'datatype': list,
        },
        'name': {
    ...
    'Servicegroup': {
        ...
        'members_with_state': {
            ...
            'function': lambda item, req: [sep_join(req, x.host_name, x.service_description, x.state_id, x.has_been_checked) for x in sorted(item.members, key=lambda y: y.get_full_name())],
            'datatype': list,
        },
        'name': {
    ...
    'Contact': {
        ...
        'in_host_notification_period': {
            ...
            'function': lambda item, req: (item.host_notification_period is None and [False] or [item.host_notification_period.is_time_valid(time.time())])[0],
            'datatype': bool,
        },
        'in_service_notification_period': {
            'description': 'Whether the contact is currently in his/her service notification period (0/1)',
            'function': lambda item, req: (item.service_notification_period is None and [False] or [item.service_notification_period.is_time_valid(time.time())])[0],
            'datatype': bool,
        },
        'modified_attributes': {
            'description': 'A bitmask specifying which attributes have been modified',
...
object_class_map = {
    ...
    None: ('', type('commandclass', (object,), {'wui_columns': []})),
}
...
def host_redirect_factory(attribute):
    """attribute already comes with wui_"""
...
for objtype in ['Host', 'Service', 'Contact', 'Command', 'Timeperiod', 'Downtime', 'Comment', 'Hostgroup', 'Servicegroup', 'Contactgroup', 'SchedulerLink', 'PollerLink', 'ReactionnerLink', 'BrokerLink', 'Problem', 'Logline', 'Config']:
    cls = [t[1] for t in object_class_map.values() if t[0] == objtype][0]
    setattr(cls, 'livestatus_attributes', [])
    for attribute in dm_mapping[objtype]:
        entry = dm_mapping[objtype][attribute]
        
        # Default is to store attribute in API in not specified in datamapping ...
        # setattr(cls, 'api_'+attribute, False)
        # if 'api' in entry:
            # cls['api_'+attribute] = entry['api']
            
        if 'function' in entry:
            setattr(cls, 'wui_'+attribute, entry['function'])
            if 'datatype' in entry:
                #getattr(cls, 'wui_'+attribute).im_func.datatype = entry['datatype']
                getattr(cls, 'wui_'+attribute).im_func.datatype = entry['datatype']
                ...
                getattr(cls, 'wui_'+attribute).im_func.datatype = int
            else:
                getattr(cls, 'wui_'+attribute).im_func.datatype = str
        elif objtype == 'Service' and attribute.startswith('host_'):
            # Service,host_address -> Service.wui_host_address -> Host.wui_address
            setattr(cls, 'wui_'+attribute, host_redirect_factory('wui_'+attribute.replace('host_', '')))
            # Service.wui_host_address.datatype = Host.wui_address.datatype
            getattr(cls, 'wui_'+attribute).im_func.datatype = getattr(Host, 'wui_'+attribute.replace('host_', '')).datatype
        elif (objtype == 'Comment' or objtype == 'Downtime') and attribute.startswith('host_'):
            # Downtime,host_address -> Downtime.wui_host_address -> "Ref".wui_host_address
            #   ref is a host: Host.wui_host_address works, because all wui_* also exist as wui_host_*
            #   ref is a service: Service.wui_host_address works, because it's delegated to the service's host
            setattr(cls, 'wui_'+attribute, ref_redirect_factory('wui_'+attribute))
            getattr(cls, 'wui_'+attribute).im_func.datatype = getattr(Host, 'wui_'+attribute).datatype
        elif (objtype == 'Comment' or objtype == 'Downtime') and attribute.startswith('service_'):
            # Downtime,service_state -> Downtime.wui_service_state -> "Ref".wui_state
            #   ref is a host: Host.wui_state works, although it is wrong. other service-only attributes return 0
            #   ref is a service: Service.wui_state works
            setattr(cls, 'wui_'+attribute, ref_redirect_factory('wui_'+attribute.replace('service_', '')))
            getattr(cls, 'wui_'+attribute).im_func.datatype = getattr(Service, 'wui_'+attribute.replace('service_', '')).datatype
        elif objtype == 'Logline' and attribute.startswith('current_service_'):
            setattr(cls, 'wui_'+attribute, log_service_redirect_factory('wui_'+attribute.replace('current_service_', '')))
            getattr(cls, 'wui_'+attribute).im_func.datatype = getattr(Service, 'wui_'+attribute.replace('current_service_', '')).datatype
        elif objtype == 'Logline' and attribute.startswith('current_host_'):
            setattr(cls, 'wui_'+attribute, log_host_redirect_factory('wui_'+attribute.replace('current_host_', '')))
            getattr(cls, 'wui_'+attribute).im_func.datatype = getattr(Host, 'wui_'+attribute.replace('current_host_', '')).datatype
        elif objtype == 'Logline' and attribute.startswith('current_contact_'):
            setattr(cls, 'wui_'+attribute, log_contact_redirect_factory('wui_'+attribute.replace('current_contact_', '')))
            getattr(cls, 'wui_'+attribute).im_func.datatype = getattr(Contact, 'wui_'+attribute.replace('current_contact_', '')).datatype
        else:
            pass
            # let the lambda return a default value
            # setattr(cls, 'wui_'+attribute, lambda item, req: 0)
            # getattr(cls, 'wui_'+attribute).im_func.datatype =?
        getattr(cls, 'wui_'+attribute).im_func.description = entry['description']
        # _Every_ attribute _must_ have a description
    if objtype == 'Host':
        # for every wui_* there is also a wui_host_*
        for attribute in dm_mapping['Host']:
            setattr(cls, 'wui_host_'+attribute, getattr(cls, 'wui_'+attribute))
            getattr(cls, 'wui_host_'+attribute).im_func.description = getattr(cls, 'wui_'+attribute).im_func.description
            getattr(cls, 'wui_host_'+attribute).im_func.datatype = getattr(cls, 'wui_'+attribute).im_func.datatype
        # this is for "GET downtimes\nFilter: service_description !=\n" which is used to fetch host-downtimes
        setattr(cls, 'wui_description', lambda item, ref: '')
...
for objtype in ['Host', 'Service']:
    ...
    if objtype == 'Host':
        ...
        for attribute in dm_mapping['Hostgroup']:
            setattr(cls, 'wui_hostgroup_'+attribute, hostgroup_redirect_factory('wui_'+attribute.replace('hostgroup_', '')))
            getattr(cls, 'wui_hostgroup_'+attribute).im_func.description = getattr(Hostgroup, 'wui_'+attribute).im_func.description
            getattr(cls, 'wui_hostgroup_'+attribute).im_func.datatype = getattr(Hostgroup, 'wui_'+attribute).im_func.datatype
    if objtype == 'Service':
        for attribute in dm_mapping['Servicegroup']:
            setattr(cls, 'wui_servicegroup_'+attribute, servicegroup_redirect_factory('wui_'+attribute.replace('servicegroup_', '')))
            getattr(cls, 'wui_servicegroup_'+attribute).im_func.description = getattr(Servicegroup, 'wui_'+attribute).im_func.description
            getattr(cls, 'wui_servicegroup_'+attribute).im_func.datatype = getattr(Servicegroup, 'wui_'+attribute).im_func.datatype
...
for objtype in ['Host', 'Service', 'Contact', 'Command', 'Timeperiod', 'Downtime', 'Comment', 'Hostgroup', 'Servicegroup', 'Contactgroup', 'SchedulerLink', 'PollerLink', 'ReactionnerLink', 'BrokerLink', 'Problem', 'Logline', 'Config']:
    ...
    for attribute in dm_mapping[objtype]:
        # entry =  dm_mapping[objtype][attribute]
        if not hasattr(getattr(cls, 'wui_'+attribute).im_func, 'datatype'):
            getattr(cls, 'wui_'+attribute).im_func.default = 0
        elif getattr(cls, 'wui_'+attribute).im_func.datatype == int:
            getattr(cls, 'wui_'+attribute).im_func.default = 0
        elif getattr(cls, 'wui_'+attribute).im_func.datatype == float:
            getattr(cls, 'wui_'+attribute).im_func.default = 0.0
        elif getattr(cls, 'wui_'+attribute).im_func.datatype == str:
            getattr(cls, 'wui_'+attribute).im_func.default = ''
        elif getattr(cls, 'wui_'+attribute).im_func.datatype == list:
            getattr(cls, 'wui_'+attribute).im_func.default = []
        elif getattr(cls, 'wui_'+attribute).im_func.datatype == bool:
            getattr(cls, 'wui_'+attribute).im_func.default = False

for objtype in ['Host', 'Service', 'Contact', 'Command', 'Timeperiod', 'Downtime', 'Comment', 'Hostgroup', 'Servicegroup', 'Contactgroup', 'SchedulerLink', 'PollerLink', 'ReactionnerLink', 'BrokerLink', 'Problem', 'Logline', 'Config']:
    cls = [t[1] for t in object_class_map.values() if t[0] == objtype][0]
    cls.wui_columns = []
    for attribute in sorted([x for x in dm_mapping[objtype]]):
        cls.wui_columns.append(attribute)
...
def find_filter_converter(table, attribute, reverse=False):
    ...
    tableclass = object_class_map[table][1]
...
def list_livestatus_attributes(table):
    tableclass = object_class_map[table][0]
    return sorted(dm_mapping[tableclass].keys())
